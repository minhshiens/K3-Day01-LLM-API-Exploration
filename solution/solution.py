...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    pricing = PRICING_PER_1K_TOKENS.get(model, PRICING_PER_1K_TOKENS["gpt-4o"])
    input_cost = count_tokens(prompt, model) / 1000 * pricing["input"]
    output_cost = count_tokens(response, model) / 1000 * pricing["output"]
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        print()  # Xuống dòng sau khi assistant trả lời xong

        history.append({"role": "assistant", "content": reply})

        # Giữ lại 3 lượt gần nhất (6 message)
        if len(history) > 6:
            history = history[-6:]
# ---------------------------------------------------------------------------
...
def run_assistant(
    ...
    while True:
        ...
        messages = [{"role": "system", "content": persona}] + history \
                   + [{"role": "user", "content": user_msg}]
        # Gọi API với stream=True, bọc trong retry_with_backoff
        ...
        for chunk in stream:
            ...
            reply += delta
        print()
        # Cập nhật history (tối đa 3 lượt, tức 6 message)
        history.append({"role": "user", "content": user_msg})
        history.append({"role": "assistant", "content": reply})
        history = history[-6:]
        # Cập nhật thống kê
